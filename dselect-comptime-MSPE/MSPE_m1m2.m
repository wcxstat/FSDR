%%%%%%%%%% Simulation codes for MSPE %%%%%%%%%%
% Ten estimators for Models 7.1-7.2

alpha=1.1;
n=200;
% n=500;

%%% H is the number of slices: H=5 for n=200; H=10 for n=500 %%%
%%% H1 is the number of slices for FMS %%%
H=5;H1=5;
% H=10;H1=5;

arg=0:0.01:1;
h=arg(2)-arg(1);
L=length(arg);
TR=diag([0.5,ones([1,L-2]),0.5]*h);
PHI=[ones(1,L);sqrt(2)*cos((1:49)'*arg*pi)]; % 50-by-L matrix
beta1_coef=4*(-1).^(2:51).*(1:50).^(-2);
beta2_coef=4*(1:50).^(-2);
beta=PHI'*[beta1_coef',beta2_coef']; % two true slope functions
d=size(beta,2);
Ps=proj(beta,arg);

n_test=1000;
N=n+n_test;
svec=d:(d+5);
MSPE=zeros(1000,length(svec),10);
parfor rep=1:1000
    Lambda=zeros(L,L,9);
    %%% Normally distributed Z_k' %%%
    Z_N=normrnd(0,1,N,50);

    %%% Multivariate t distributed Z_k' %%%
    % z_N=normrnd(0,1,N,50);
    % tau_N=chi2rnd(5,[1,N]);
    % Z_N=z_N./repmat(sqrt(tau_N'/(5-2)),1,50);

    %%% Uniformly distributed Z_k' %%%
    % Z_N=unifrnd(-sqrt(3),sqrt(3),[N,50]);

    eigvalsq=(-1).^(2:51).*(1:50).^(-alpha/2);
    score_N=Z_N.*repmat(eigvalsq,N,1);
    X_N=score_N*PHI;
    Xbeta1=(beta1_coef.*eigvalsq)*Z_N';
    Xbeta2=(beta2_coef.*eigvalsq)*Z_N';

    p=4;
    mu=zeros(1,p);
    Sigma=0.5.^(toeplitz(0:(p-1)));
    epsilon_N=0.1*mvnrnd(mu,Sigma,N);
    Y_N=[Xbeta1',Xbeta2',zeros(N,2)]+epsilon_N; % Model 1
    % Y_N=[10*Xbeta1'./(9+(Xbeta2'+1).^2),10./(4+sin(Xbeta1')),zeros(N,2)]+epsilon_N; % Model 2
    
    X=X_N(1:n,:);Y=Y_N(1:n,:); % training data
    X_test=X_N((n+1):end,:);Y_test=Y_N((n+1):end,:); % testing data

    no_pca=15;
    [kappa,eigfun,xi_est]=FPCA_bal(X,arg,no_pca);

    %%%%%%%%%%%%%%%% PFSIR %%%%%%%%%%%%%%%%
    mn=ceil(n^(3/2));
    G=normrnd(0,1,mn,p);
    Lambda_PFSIR=0;
    for j=1:mn
        W=G(j,:)/norm(G(j,:));
        Lambda_PFSIR=Lambda_PFSIR+FSIR(W*Y',X,H);
    end
    Lambda(:,:,1)=Lambda_PFSIR/mn;

    %%%%%%%%%%%%%%%% PFSAVE %%%%%%%%%%%%%%%%
    G=normrnd(0,1,mn,p);
    Lambda_PFSAVE=0;
    for j=1:mn
        W=G(j,:)/norm(G(j,:));
        Lambda_PFSAVE=Lambda_PFSAVE+FSAVE(W*Y',X,arg,H);
    end
    Lambda(:,:,2)=Lambda_PFSAVE/mn;

    %%%%%%%%%%%%%%%% PFCS and Distance %%%%%%%%%%%%%%%%
    sigma=median(pdist(Y));
    Xc=X-mean(X);
    Lad_PFCS=0;
    Lad_Sq=0;Lad_Abs=0;Lad_GK=0;Lad_LK=0;
    for i=1:(n-1)
        for j=(i+1):n
            index=setdiff(1:n,[i,j]);
            Yij=Y(index,:);
            angdist_ij=sum(Ang(Y(i,:)-Yij,Y(j,:)-Yij))/(pi*n);
            Lad_PFCS=Lad_PFCS+angdist_ij*Xc(i,:)'*Xc(j,:);
            
            dist_ij=norm(Y(i,:)-Y(j,:));
            Lad_Sq=Lad_Sq+dist_ij*Xc(i,:)'*Xc(j,:);
            Lad_Abs=Lad_Abs+norm(Y(i,:)-Y(j,:),1)*Xc(i,:)'*Xc(j,:);
            Lad_GK=Lad_GK+(2-2*exp(-dist_ij^2/(2*sigma^2)))*Xc(i,:)'*Xc(j,:);
            Lad_LK=Lad_LK+(2-2*exp(-dist_ij/sigma))*Xc(i,:)'*Xc(j,:);
        end
    end
    Lambda(:,:,3)=-(Lad_PFCS+Lad_PFCS')/(2*n^2);
    Lambda(:,:,4)=-(Lad_Sq+Lad_Sq')/(n^2);
    Lambda(:,:,5)=-(Lad_Abs+Lad_Abs')/(n^2);
    Lambda(:,:,6)=-(Lad_GK+Lad_GK')/(n^2);
    Lambda(:,:,7)=-(Lad_LK+Lad_LK')/(n^2);
    
    %%%%%%%%%%%%%%%% FMS %%%%%%%%%%%%%%%%
    Lambda(:,:,8)=MS_FSIR(Y,X,H1);
    
    %%%%%%%%%%%%%%%% FMC %%%%%%%%%%%%%%%%    
    Lambda_FMC=0;
    for j=1:p
        Lambda_FMC=Lambda_FMC+FSIR(Y(:,j)',X,H);
    end
    Lambda(:,:,9)=Lambda_FMC/p;
    
    MSPE1=zeros(length(svec),10);
    for k=1:9
        [~,psi_est]=inteq(Lambda(:,:,k),arg,d);
        for sn=1:length(svec)
            beta_est=linv(arg,kappa,eigfun,psi_est,svec(sn));
            bX=zeros(n,d);
            bX_test=zeros(n_test,d);
            for dd=1:d
                bX(:,dd)=trapz(arg,X'.*repmat(beta_est(:,dd),1,n))';
                bX_test(:,dd)=trapz(arg,X_test'.*repmat(beta_est(:,dd),1,n_test))';
            end
            ypred=zeros(n_test,p);
            for j=1:p
                h1=opt_h(bX,Y(:,j),'kr');
                ypred(:,j)=ks(bX,Y(:,j),'kr',h1,bX_test);
            end
            if k==8||k==9
                MSPE1(sn,k+1)=mean(sum((Y_test-ypred).^2,2));
            else
                MSPE1(sn,k)=mean(sum((Y_test-ypred).^2,2));
            end
        end
    end

    %%%%%%%%%%%%%%%% FKIR %%%%%%%%%%%%%%%%
    KImat=FKIR(Y,X,H);
    for sn=1:length(svec)
        s=svec(sn);
        xcov_inv_sq=eigfun(:,1:s)*diag(kappa(1:s).^(-1/2))*eigfun(:,1:s)';
        Gamma1=xcov_inv_sq*TR*KImat*TR*xcov_inv_sq;
        Gamma=(Gamma1+Gamma1')/2;
        [eigen,eigv]=eigs(Gamma,d,'lm');
        beta_FKIR=xcov_inv_sq*eigen;
        bX=zeros(n,d);
        bX_test=zeros(n_test,d);
        for dd=1:d
            bX(:,dd)=trapz(arg,X'.*repmat(beta_FKIR(:,dd),1,n))';
            bX_test(:,dd)=trapz(arg,X_test'.*repmat(beta_FKIR(:,dd),1,n_test))';
        end
        ypred=zeros(n_test,p);
        for j=1:p
            h1=opt_h(bX,Y(:,j),'kr');
            ypred(:,j)=ks(bX,Y(:,j),'kr',h1,bX_test);
        end
        MSPE1(sn,8)=mean(sum((Y_test-ypred).^2,2));
    end
    MSPE(rep,:,:)=MSPE1;
end

MSPEmat=zeros(10,4);
for k=1:10
    MSPE_rm=rmmissing(MSPE(:,:,k));
    Mt=mean(MSPE_rm,1);
    [minValue,pt]=min(Mt);
    MSPEmat(k,:)=[minValue,std(MSPE_rm(:,pt)),svec(pt),length(MSPE_rm)];
end

