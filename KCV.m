function CV=KCV(arg,X,Y,svec,dvec,K,H)
% Using K-fold cross-validation to select (d, sn) of ten estimators
% H1=5 is the number of slices for FMS

% Input
% svec: candidate truncation parameters
% dvec: candidate structural dimensions
% H: number of slices for PFSIR, PFSAVE, and FMC, or number of clusters for FKIR

% Output
% CV, where d_hat=CV(:,1), s_hat=CV(:,2), and minCV=CV(:,3)

% Copyright: Wenchao Xu at SUIBE, Oct., 2025.
...
[n,p]=size(Y);
M=floor(n/K);
MSPE=10000*ones(length(svec),length(dvec),K,10);
for kk=1:K
    if kk==K
        index=((K-1)*M+1):n;
    else
    ...
    for k=1:9
        for d1=2:length(dvec)
            ...
            for sn=d:length(svec)
                ...
                if k==8||k==9
                    MSPE(sn,d1,kk,k+1)=mean(sum((Y_test-ypred).^2,2));
                else
        ...
        if k==8||k==9
            MSPE(:,1,kk,k+1)=repmat(mean(sum((Y_test-mean(Y_train)).^2,2)),length(svec),1);
        else
    ...
    for d1=2:length(dvec)
        ...
        for sn=d:length(svec)
            ...
            MSPE(sn,d1,kk,8)=mean(sum((Y_test-ypred).^2,2));
        end
    end
    MSPE(:,1,kk,8)=repmat(mean(sum((Y_test-mean(Y_train)).^2,2)),length(svec),1);
end
